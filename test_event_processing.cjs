...
(async () => {
  const diagram = new Diagram({
    ...
    filename: '/tmp/aws_event_processing_fixed',
    show: false,
    outformat: 'png',
  });
  ...
  await diagram.use(async () => {
    ...
    workers.forEach(worker => source.to(worker));
    workers.forEach(worker => worker.to(queue));
    handlers.forEach(handler => queue.to(handler));
